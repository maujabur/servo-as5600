...
.save-wrap{padding:18px 20px 28px}.save{width:100%;min-height:62px;background:var(--cyan);color:#061116}.message{min-height:24px;margin-top:13px;color:var(--green);font:13px monospace;text-align:center}.message.error{color:#ff6565}
...
<main>
  ...
  <form id="config">
    <section><h2>Limites</h2><div class="grid">
      <div class="field"><label for="start">Start deg</label><div class="input"><input id="start" name="start" type="number" min="0" max="359.99" step="0.1" required><span class="unit">°</span></div><div class="hint">0.0 – 359.9°</div></div>
      <div class="field"><label for="end">End deg</label><div class="input"><input id="end" name="end" type="number" min="0" max="359.99" step="0.1" required><span class="unit">°</span></div><div class="hint">0.0 – 359.9°</div></div>
    </div></section>
    ...
    <div class="save-wrap"><button class="save" type="submit">APLICAR PARÂMETROS</button><div class="message" id="message">Configuração persistente</div></div>
  </form>
</main>
<script>
...
function render(s){const phases={STOPPED:'EM REPOUSO',TO_END:'INDO AO FIM',DWELL_END:'PAUSA NO FIM',TO_START:'INDO AO INÍCIO',DWELL_START:'PAUSA NO INÍCIO'};last={start:s.start,end:s.end,angle:s.angle||0};$('phase').textContent=phases[s.phase]||s.phase;$('angle').textContent=s.sensor?s.angle.toFixed(1)+'°':'SEM SENSOR';$('run').disabled=s.running||s.otaBusy;$('stop').disabled=!s.running;$('ota').textContent=s.otaBusy?'OTA/AP: UPDATE':'OTA/AP: IDLE';if(!editing){$('start').value=s.start.toFixed(1);$('end').value=s.end.toFixed(1);$('rpmOut').value=s.rpmOut.toFixed(2);$('rpmBack').value=s.rpmBack.toFixed(2);$('waitStart').value=s.waitStart;$('waitEnd').value=s.waitEnd;$('rpmOut').max=s.maxRpm;$('rpmBack').max=s.maxRpm}drawGauge()}
